...
def eh_download_info():
    return hpx.command.DownloadInfo(
        ...
        name="E-Hentai",
        parser=website_url_regex_gen(
            "e-hentai.org",
            path_regex=r"g\/[0-9]{3,10}\/[0-9a-zA-Z]{3,15}",
            trailing_slash=True,
            variable_tld=False,
            trailing_fragment=True,
            end=True,
...
def download_query(item, is_exhentai):
    ...
    if login_session:
        ...
        req_props = hpx.command.RequestProperties(
            headers=HEADERS, json=eh_data, session=login_session
        )
        ...
        if r.ok:
            try:
                try:
                    ...
                    response = None
                    log.info(
                        "got empty response when trying to retrieve archiver key, this usually means that user has no access to exhentai"
                    )
                if response and "error" not in response:
                    for gdata in response["gmetadata"]:
                        if all(
                            ...
                            req_props = hpx.command.RequestProperties(
                                headers=HEADERS, data=form_data, session=login_session
                            )
                            r = hpx.command.SinglePOSTRequest().request(
                                a_url, req_props
                            )
                            if (
                                r.ok
                                and "Key missing, or incorrect key provided"
                                not in r.text
                            ):
                                ...
                                log.warning(
                                    f"got invalid key page or bad status: {r.status_code}"
                                )
